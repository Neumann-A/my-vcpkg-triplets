...
if(VCPKG_USE_SANITIZERS)
    string(APPEND CLANG_FLAGS_RELEASE "-fsanitize=undefined,null,alignment,address -fsanitize-stats")
endif()
...
#set(CMAKE_LINKER "${CLANG-CL_EXECUTBALE}" CACHE STRING "") # TODO
...
set(CMAKE_C_FLAGS_RELEASE "/O2 /Oi /Ob2 /GS- ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} ${VCPKG_DBG_FLAG} /DNDEBUG ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
set(CMAKE_C_FLAGS_MINSIZEREL "/O1 /Oi /Ob1 /GS- ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} /DNDEBUG ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
set(CMAKE_C_FLAGS_RELWITHDEBINFO "/O2 /Oi /Ob1 /GS- ${VCPKG_C_FLAGS_RELEASE} ${VCPKG_CRT_FLAG} ${CLANG_C_LTO_FLAGS} ${VCPKG_DBG_FLAG} /DNDEBUG ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
...
set(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_C_FLAGS_RELEASE} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS} ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
set(CMAKE_CXX_FLAGS_MINSIZEREL "${CMAKE_C_FLAGS_MINSIZEREL} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS} ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
set(CMAKE_CXX_FLAGS_RELWITHDEBINFO "${CMAKE_C_FLAGS_RELWITHDEBINFO} ${VCPKG_CXX_FLAGS_RELEASE} ${CLANG_CXX_LTO_FLAGS} ${CLANG_FLAGS_RELEASE}" CACHE STRING "")
...
foreach(linker IN ITEMS "SHARED_LINKER" "MODULE_LINKER" "EXE_LINKER")
  ...
  set(CMAKE_${linker}_FLAGS "${CMAKE_CL_NOLOGO} /INCREMENTAL:NO /Brepro ${VCPKG_LINKER_FLAGS}" CACHE STRING "")
  ...
  message(STATUS "Setting up:${linker}")
endforeach()

foreach(lang IN ITEMS C CXX)
  foreach(linker IN ITEMS "SHARED_LINKER" "MODULE_LINKER" "EXE_LINKER")
    set(CMAKE_${lang}_${linker}_FLAGS "${CMAKE_${linker}_FLAGS}" CACHE STRING "")
    set(CMAKE_${lang}_${linker}_FLAGS_DEBUG "${CMAKE_${linker}_FLAGS_DEBUG}" CACHE STRING "")
    set(CMAKE_${lang}_${linker}_FLAGS_RELEASE "${CMAKE_${linker}_FLAGS_RELEASE}" CACHE STRING "")
    set(CMAKE_${lang}_${linker}_FLAGS_MINSIZEREL "${CMAKE_${linker}_FLAGS_MINSIZEREL}" CACHE STRING "")
    set(CMAKE_${lang}_${linker}_FLAGS_RELWITHDEBINFO "${CMAKE_${linker}_FLAGS_RELWITHDEBINFO}" CACHE STRING "")
    message(STATUS "Setting up:${lang}_${linker}")
  endforeach()
endforeach()
unset(linker)
unset(lang)
...
#  ___windows_compiler_msvc(${lang})
#  set(CMAKE_${lang}_CREATE_SHARED_LIBRARY
#    "${_CMAKE_VS_LINK_DLL}<CMAKE_${lang}_COMPILER> ${CMAKE_CL_NOLOGO} <LANGUAGE_COMPILE_FLAGS> <OBJECTS> /link ${CMAKE_START_TEMP_FILE} /out:<TARGET> /implib:<TARGET_IMPLIB> /pdb:<TARGET_PDB> /dll #/version:<TARGET_VERSION_MAJOR>.<TARGET_VERSION_MINOR>${_PLATFORM_LINK_FLAGS} <LINK_FLAGS> <LINK_LIBRARIES> ${CMAKE_END_TEMP_FILE}")
